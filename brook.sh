...
版本列表请去这里获取：${Green_font_prefix}[ https://github.com/txthinking/brook/releases ]${Font_color_suffix}"
	...
	if [[ -z ${brook_new_ver} ]]; then
		brook_new_ver=$(wget -qO- https://928-download.oss-cn-hongkong.aliyuncs.com/brook)
...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "https://928-download.oss-cn-hongkong.aliyuncs.com/brook"
	else
